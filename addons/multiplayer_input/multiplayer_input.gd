...
func reset():
	...
	# I can't just delete them because they're used as blueprints when a joypad connects
...
func get_action_name(device: int, action: StringName) -> StringName:
	...
	if device >= 0:
		assert(
			...
			"Device %s has no actions. Maybe the joypad is disconnected." % device
		)
...
## NOTE: this calls reset(), so if you make any changes to the InputMap via code,
## you'll need to make them again.
